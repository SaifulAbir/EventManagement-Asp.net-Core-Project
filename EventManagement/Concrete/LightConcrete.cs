...
namespace EventManagement.Concrete
{
    public class LightConcrete : ILight
    {
        ...
        public IQueryable<LightViewModel> ShowLight(string sortColumn, string sortColumnDir, string Search)
        {
            var IQueryableLight = (from tempLight in _context.Light
                                   select new LightViewModel
                                   {
                                       ...
                                       Createdate = tempLight.Createdate.Value.ToString("dd/MM/yyyy")
                                   });
